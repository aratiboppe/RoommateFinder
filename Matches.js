...
const Matches = () => {
  ...
  const dislikeMatch = async (User2Name) => {
    try {
      ...
      // Correcting the call for dislike status
      await updateStatus(User2Name, "No", "Yes");
    } catch (error) {
  ...
  //   try {
  //     // Send the updated status to the database
  //     await axios.post(`http://localhost:3001/matches/${User2Name}`, {
  //       method: "POST",
  //       headers: {
  //         "Content-Type": "application/json",
  //       },
  //       body: JSON.stringify({ LikeStatus: likeStatus, DislikeStatus: dislikeStatus })
  //     });
  //   } catch (error) {
  //     console.error("Error updating status:", error);
  //   }
  ...
  const renderItem = ({ item }) => (
    ...
    <Modal visible={selectedMatch === item} animationType="slide" transparent={true}>
      <View style={styles.modalContainer}>
        <ScrollView contentContainerStyle={styles.modalContent}>
          {selectedMatch && (
            <View style={styles.preferencesContainer}>
              <Text style={styles.preferenceName}>{selectedMatch.User2Name}'s Preferences</Text>
              ...
              <Text style={styles.preferenceLabel}>Budget: {selectedMatch.User2Budget}</Text>
      ...
      <Pressable
        ...
        onPress={() => likeMatch(item.User2Name)}
      >
        <View style={[styles.iconContainer, item.LikeStatus && styles.likedIconContainer]}>
          <Image
      ...
      <Pressable
        ...
        onPress={() => dislikeMatch(item.User2Name)}
      >
        <View style={[styles.iconContainer, item.DislikeStatus && styles.dislikedIconContainer]}>
          <Image
